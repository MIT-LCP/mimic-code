...
WITH vasocv1 AS (
  SELECT
    cv.icustay_id,
    cv.charttime,
    MAX(CASE WHEN itemid IN (30047, 30120) THEN 1 ELSE 0 END) AS vaso,
    MAX(
      CASE
  ...
  WHERE
    itemid IN (30047, 30120) AND NOT cv.icustay_id IS NULL
  GROUP BY
    cv.icustay_id,
    cv.charttime
), vasocv2 AS (
  ...
  WHERE
    itemid = 221906 AND statusdescription <> 'Rewritten'
)
SELECT
  icustay_id,
  starttime,
