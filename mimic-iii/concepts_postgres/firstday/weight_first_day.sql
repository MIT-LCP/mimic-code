...
/* This query extracts weights for adult ICU patients on their first ICU day. */ /* It does *not* use any information after the first ICU day, as weight is */ /* sometimes used to monitor fluid balance. */ /* ** Requires the echodata view, generated by concepts/echo-data.sql */
WITH ce AS (
  SELECT
    c.icustay_id, /* we take the avg value from roughly first day */ /* TODO: eliminate obvious outliers if there is a reasonable weight */ /* (e.g. weight of 180kg and 90kg would remove 180kg instead of taking the median) */
  ...
  GROUP BY
    c.icustay_id
), echo_hadm /* we split in-hospital/out of hospital echoes as we would like to prioritize in-hospital data */ AS (
  SELECT
    ie.icustay_id,
