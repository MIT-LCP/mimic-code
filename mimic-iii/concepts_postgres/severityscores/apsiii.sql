...
WITH pa AS (
  ...
  WHERE
    vd.icustay_id IS NULL /* patient is *not* ventilated */
  ...
  SELECT
    ...
    bg.charttime,
    bg.aado2,
    ROW_NUMBER() OVER (PARTITION BY bg.icustay_id ORDER BY bg.aado2 DESC NULLS LAST) AS rn
  ...
  INNER JOIN mimiciii_derived.ventilation_durations AS vd
    ...
    AND bg.charttime <= vd.endtime
  WHERE
    NOT vd.icustay_id IS NULL /* patient is ventilated */
  ...
  SELECT
    ...
    ROW_NUMBER() OVER (PARTITION BY icustay_id ORDER BY acidbase_score DESC NULLS LAST) AS acidbase_rn
  FROM acidbase
  ...
  SELECT
    ...
    gcs.gcsverbal,
    gcs.gcseyes,
    gcs.endotrachflag, /* acute renal failure */
    arf.arf AS arf
  FROM mimiciii.icustays AS ie
  ...
  SELECT
    ...
    CASE
      ...
      WHEN meanbp_min >= 140
      THEN 10
    END AS meanbp_score, /* TODO: add 1 degree to axillary measurements */
    CASE
