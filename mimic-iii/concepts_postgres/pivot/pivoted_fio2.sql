...
WITH pvt AS (
  SELECT
    le.hadm_id,
    le.charttime, /* here we assign labels to ITEMIDs */ /* this also fuses together multiple ITEMIDs containing the same data */ /* add in some sanity checks on the values */
    ROUND(CAST(MAX(
      CASE
        WHEN valuenum <= 0
        THEN NULL
  ...
  SELECT
    ...
    ROUND(CAST(MAX(
      CASE
        ...
        ELSE NULL
      END
    ) AS NUMERIC), 2) AS fio2_chartevents
