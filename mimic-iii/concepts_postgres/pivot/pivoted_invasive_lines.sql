...
WITH stg0 AS (
  SELECT
    ...
    charttime,
    storetime,
    itemid, /* create partition which separates the lines */
    CASE
  ...
  SELECT
    ...
    line_type,
    line_site,
    line_dc, /* only keep the last documented value */
    ROW_NUMBER() OVER (PARTITION BY icustay_id, charttime, itemid ORDER BY storetime DESC NULLS LAST) AS rn_last_stored
  FROM stg0
), stg1 AS (
  SELECT
    icustay_id,
    charttime,
    line_number, /* collapse line type/site into a single row */ /* MAX() always collapses a single value, due to where rn_last_stored = 1 */
    MAX(line_type) AS line_type,
    MAX(line_site) AS line_site, /* any disconnection at this charttime turns off the line */
  ...
  SELECT
    ...
    line_type,
    line_site,
    line_dc, /* carry forward the line type */
    CASE
      ...
      AND CAST(EXTRACT(EPOCH FROM DATE_TRUNC('hour', charttime) - DATE_TRUNC('hour', LAG(charttime) OVER (PARTITION BY icustay_id, line_number ORDER BY charttime NULLS FIRST))) / 3600 AS BIGINT) < 16
      THEN 0
      ELSE 1
    END AS rn_part
  FROM stg1
), stg3 /* rn_part is 1 if it's a new event, and 0 if it's a continuation of the previous */ /* so cumulatively summing it will result in a sequential integer which partitions */ /* distinct line events. we can later group on this integer. */ AS (
  SELECT
    icustay_id,
    charttime,
    line_number,
  ...
  FROM stg2
), stg4 /* group by line_event to determine line start/stop times */ AS (
  SELECT
    icustay_id,
    line_number,
    line_event,
  ...
  SELECT
    icustay_id, /* since metavision separates lines using itemid, we can use it as the line number */
