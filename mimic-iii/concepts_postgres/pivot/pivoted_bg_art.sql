...
WITH stg_spo2 AS (
  ...
  SELECT
    bg.*,
    ROW_NUMBER() OVER (PARTITION BY bg.hadm_id, bg.charttime ORDER BY s1.charttime DESC NULLS LAST) AS lastrowspo2,
    s1.spo2
  ...
  SELECT
    bg.*,
    ROW_NUMBER() OVER (PARTITION BY bg.hadm_id, bg.charttime ORDER BY s2.charttime DESC NULLS LAST) AS lastrowfio2,
