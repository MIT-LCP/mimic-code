...
WITH ce AS (
  SELECT
    ...
    CASE
      ...
      THEN 1 /* "Dialysate Infusing";56605 */
  ...
  WHERE
    ce.itemid IN (
      ...
      150, /* "Dialysis Machine";27472 (baxter or gambro) */
      151, /* "Dialysis Site Appear";37345 */
      582, /* Procedures */ /* below indicate existence of a dialysis line */
      229, /* INV Line#1 [Type] */
      235, /* INV Line#2 [Type] */
      241, /* INV Line#3 [Type] */
      247, /* INV Line#4 [Type] */
      ...
      259, /* INV Line#6 [Type] */
      265, /* INV Line#7 [Type] */
      271, /* INV Line#8 [Type] */ /* dialysis consults can't be 100% guaranteed to be active */ /* ,466 -- Nursing Consultation */ /* diagnosis has 6 or 7 dx related to dialysis, probably not worth including */ /* as the chart time isn't going to match the start time of dialysis */ /* , 917 -- Diagnosis/op */ /* ,7949 -- "Calcium for CVVH" - only has 2 null values */ /* === MetaVision itemids === -- */ /* Checkboxes */
      226118, /* | Dialysis Catheter placed in outside facility      | Access Lines - Invasive | chartevents        | Checkbox */
      227357, /* | Dialysis Catheter Dressing Occlusive              | Access Lines - Invasive | chartevents        | Checkbox */
      ...
      225959, /* | Medication Added Amount  #1 (Peritoneal Dialysis) | Dialysis | chartevents | Numeric */ /* Text values */
    ...
    AND /* exclude rows marked as error */ COALESCE(ce.error, 0) = 0
), cv_ie /* TODO: */ /*   charttime + dialysis_present + dialysis_active */ /*  for inputevents_cv, outputevents */ /*  for procedures_mv, left join and set the dialysis_type */ AS (
  SELECT
    icustay_id,
    charttime,
    1 AS dialysis_present,
    CASE
      WHEN NOT itemid IN (44954 /* OR CVVHDF |  | inputevents_cv */)
      THEN 1
      ELSE 0
    END AS dialysis_active,
    CASE
  ...
  SELECT
    ...
    charttime,
    1 AS dialysis_present,
    CASE WHEN NOT itemid IN (41897 /* CVVH OUTPUT FROM OR */) THEN 1 ELSE 0 END AS dialysis_active,
    CASE
      WHEN itemid IN (
  ...
  SELECT
    ...
    CASE
      ...
      THEN 'IHD'
      WHEN itemid = 225802
      THEN 'CRRT' /* CVVH (Continuous venovenous hemofiltration) */
      WHEN itemid = 225803
      THEN 'CVVHD' /* CVVHD (Continuous venovenous hemodialysis) */
      WHEN itemid = 225805
      THEN 'Peritoneal'
      WHEN itemid = 225809
      THEN 'CVVHDF' /* CVVHDF (Continuous venovenous hemodiafiltration) */
      WHEN itemid = 225955
      THEN 'SCUF' /* SCUF (Slow continuous ultra filtration) */
      ELSE NULL
    END AS dialysis_type
