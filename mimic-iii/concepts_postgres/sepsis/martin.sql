...
WITH co_dx AS (
  SELECT
    ...
    MAX(
      CASE
        WHEN SUBSTRING(icd9_code FROM 1 FOR 3) = '038'
        THEN 1
        WHEN SUBSTRING(icd9_code FROM 1 FOR 4) IN ('0202', '7907', '1179', '1125')
        THEN 1
        WHEN SUBSTRING(icd9_code FROM 1 FOR 5) = '11281'
        THEN 1
        ELSE 0
      END
    ) AS sepsis,
    MAX(
      CASE
        WHEN SUBSTRING(icd9_code FROM 1 FOR 4) IN ('7991')
        THEN 1
        WHEN SUBSTRING(icd9_code FROM 1 FOR 5) IN ('51881', '51882', '51885', '78609')
        THEN 1
        ELSE 0
      END
    ) AS respiratory,
    MAX(
      CASE
        WHEN SUBSTRING(icd9_code FROM 1 FOR 4) IN ('4580', '7855', '4580', '4588', '4589', '7963')
        THEN 1
        WHEN SUBSTRING(icd9_code FROM 1 FOR 5) IN ('785.51', '785.59')
        THEN 1
        ELSE 0
      END
    ) AS cardiovascular,
    MAX(
      CASE
        WHEN SUBSTRING(icd9_code FROM 1 FOR 3) IN ('584', '580', '585')
        THEN 1
        ELSE 0
      END
    ) AS renal,
    MAX(
      CASE
        WHEN SUBSTRING(icd9_code FROM 1 FOR 3) IN ('570')
        THEN 1
        WHEN SUBSTRING(icd9_code FROM 1 FOR 4) IN ('5722', '5733')
        THEN 1
        ELSE 0
      END
    ) AS hepatic,
    MAX(
      CASE
        WHEN SUBSTRING(icd9_code FROM 1 FOR 4) IN ('2862', '2866', '2869', '2873', '2874', '2875')
        THEN 1
        ELSE 0
      END
    ) AS hematologic,
    MAX(CASE WHEN SUBSTRING(icd9_code FROM 1 FOR 4) IN ('2762') THEN 1 ELSE 0 END) AS metabolic,
    MAX(
      CASE
        WHEN SUBSTRING(icd9_code FROM 1 FOR 3) IN ('293')
        THEN 1
        WHEN SUBSTRING(icd9_code FROM 1 FOR 4) IN ('3481', '3483')
        THEN 1
        WHEN SUBSTRING(icd9_code FROM 1 FOR 5) IN ('78001', '78009')
        THEN 1
        ELSE 0
      END
    ) AS neurologic
  ...
  GROUP BY
    subject_id,
    hadm_id
), co_proc /* procedure codes: */ /* "96.7 - Ventilator management" */ /* translated: */ /*    9670	Continuous invasive mechanical ventilation of unspecified duration */ /*    9671	Continuous invasive mechanical ventilation for less than 96 consecutive hours */ /*    9672	Continuous invasive mechanical ventilation for 96 consecutive hours or more */ /* "39.95 - Hemodialysis" */ /*    3995	Hemodialysis */ /* "89.14 - Electroencephalography" */ /*    8914	Electroencephalogram */ AS (
  SELECT
    subject_id,
    hadm_id,
