...
DROP TABLE IF EXISTS mimiciii_derived.norepinephrine_durations; CREATE TABLE mimiciii_derived.norepinephrine_durations AS
/* This query extracts durations of norepinephrine administration */ /* Consecutive administrations are numbered 1, 2, ... */ /* Total time on the drug can be calculated from this table by grouping using ICUSTAY_ID */ /* Get drug administration data from CareVue first */
WITH vasocv1 AS (
  SELECT
    icustay_id,
  ...
  WHERE
    itemid IN (30047, 30120) /* norepinephrine */
  GROUP BY
    icustay_id,
    charttime
), vasocv2 AS (
  ...
  SELECT
    ...
    END AS vaso_stop
  FROM vasocv5 AS v
), vasocv /* -- if you want to look at the results of the carevue data before grouping: */ /* select */ /*   icustay_id, charttime, vaso, vaso_rate, vaso_amount */ /*     , case when vaso_stopped = 1 then 'Y' else '' end as stopped */ /*     , vaso_start */ /*     , vaso_first */ /*     , vaso_stop */ /* from vasocv6 order by charttime; */ AS (
