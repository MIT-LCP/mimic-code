...
DROP TABLE IF EXISTS mimiciii_derived.norepinephrine_dose; CREATE TABLE mimiciii_derived.norepinephrine_dose AS
/* This query extracts dose+durations of norepinephrine administration */ /* Total time on the drug can be calculated from this table by grouping using ICUSTAY_ID */ /* Get drug administration data from CareVue first */
WITH vasocv1 AS (
  SELECT
    cv.icustay_id,
    cv.charttime, /* case statement determining whether the ITEMID is an instance of vasopressor usage */
    MAX(CASE WHEN itemid IN (30047, 30120) THEN 1 ELSE 0 END) AS vaso, /* norepinephrine */ /* the 'stopped' column indicates if a vasopressor has been disconnected */
    MAX(
      CASE
        WHEN itemid IN (30047, 30120) AND (
          stopped = 'Stopped' OR stopped LIKE 'D/C%'
        )
        THEN 1
        ELSE 0
      END
    ) AS vaso_stopped, /* case statement determining whether the ITEMID is an instance of vasopressor usage */
    MAX(CASE WHEN itemid IN (30047, 30120) AND NOT rate IS NULL THEN 1 ELSE 0 END) AS vaso_null,
    MAX(
      CASE
        WHEN itemid = 30047 AND wd.weight IS NULL
        THEN CAST(rate AS DOUBLE PRECISION) / 80.0 /* this is rare, only affects a total of ~400 rows */
        WHEN itemid = 30047
        THEN CAST(rate AS DOUBLE PRECISION) / wd.weight /* measured in mcgmin */
        WHEN itemid = 30120
  ...
  WHERE
    itemid IN (30047, 30120) /* norepinephrine */ AND NOT cv.icustay_id IS NULL
  GROUP BY
    cv.icustay_id,
    cv.charttime
), vasocv2 AS (
