...
DROP TABLE IF EXISTS mimiciii_derived.vasopressin_durations; CREATE TABLE mimiciii_derived.vasopressin_durations AS
/* This query extracts durations of vasopressin administration */ /* Consecutive administrations are numbered 1, 2, ... */ /* Total time on the drug can be calculated from this table by grouping using ICUSTAY_ID */ /* Get drug administration data from CareVue first */
WITH vasocv1 AS (
  SELECT
    icustay_id,
    charttime, /* case statement determining whether the ITEMID is an instance of vasopressor usage */
    MAX(CASE WHEN itemid = 30051 THEN 1 ELSE 0 END) AS vaso, /* vasopressin */ /* the 'stopped' column indicates if a vasopressor has been disconnected */
    MAX(
      CASE
        WHEN itemid = 30051 AND (
    ...
    MAX(CASE WHEN itemid = 30051 AND NOT rate IS NULL THEN 1 ELSE 0 END) AS vaso_null,
