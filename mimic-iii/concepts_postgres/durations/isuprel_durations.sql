...
WITH vasocv1 AS (
  SELECT
    ...
    MAX(CASE WHEN itemid = 30046 THEN 1 ELSE 0 END) AS vaso, /* Isuprel */ /* the 'stopped' column indicates if a vasopressor has been disconnected */
    MAX(
      CASE
        WHEN itemid = 30046 AND (
    ...
    MAX(CASE WHEN itemid = 30046 AND NOT rate IS NULL THEN 1 ELSE 0 END) AS vaso_null,
