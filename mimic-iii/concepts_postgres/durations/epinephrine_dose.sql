...
WITH vasocv1 AS (
  SELECT
    ...
    MAX(
      CASE
        ...
        THEN CAST(rate AS DOUBLE PRECISION) / 80.0 /* super rare to be missing weight... affects 2 patients for 14 rows */
        WHEN itemid = 30044
  ...
  WHERE
    itemid IN (30044, 30119, 30309 /* epinephrine */) AND NOT cv.icustay_id IS NULL
  GROUP BY
    cv.icustay_id,
    charttime
), vasocv2 AS (
