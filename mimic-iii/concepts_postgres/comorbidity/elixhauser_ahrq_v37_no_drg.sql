...
SELECT
  ...
  CASE WHEN perivasc = 1 THEN 1 ELSE 0 END AS peripheral_vascular, /* we combine "htn" and "htncx" into "HYPERTENSION" */ /* note "htn" (hypertension) is only 1 if "htncx" (complicated hypertension) is 0 */ /* this matters if you filter on DRG but for this query we can just merge them immediately */
  CASE
    WHEN htn = 1
    THEN 1
    WHEN htncx = 1
