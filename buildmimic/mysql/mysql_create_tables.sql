...
LOAD DATA LOCAL INFILE 'DIAGNOSES_ICD_DATA_TABLE.csv' INTO TABLE DIAGNOSES_ICD
   ...
   LINES TERMINATED BY '\n'
   IGNORE 1 LINES
   (@ROW_ID,@SUBJECT_ID,@HADM_ID,@SEQUENCE,@ICD9_CODE,@DESCRIPTION)
 SET
   ROW_ID = @ROW_ID,
   SUBJECT_ID = @SUBJECT_ID,
   HADM_ID = @HADM_ID,
   SEQUENCE = IF(@SEQUENCE='', NULL, @SEQUENCE),
...
-- DROP TABLE IF EXISTS ICUSTAYEVENTS;
-- CREATE TABLE ICUSTAYEVENTS_DATA_TABLE (
--    ROW_ID SMALLINT UNSIGNED NOT NULL,
--    SUBJECT_ID MEDIUMINT UNSIGNED NOT NULL,
--    HADM_ID MEDIUMINT UNSIGNED NOT NULL,
...
LOAD DATA LOCAL INFILE 'ICUSTAYEVENTS_DATA_TABLE.csv' INTO TABLE ICUSTAYEVENTS
   FIELDS TERMINATED BY ',' ESCAPED BY '\\' OPTIONALLY ENCLOSED BY '"' 
   LINES TERMINATED BY '\n'
   IGNORE 1 LINES
...
-- CREATE TABLE IOEVENTS_DATA_TABLE (
--    ROW_ID INT UNSIGNED NOT NULL,
--    SUBJECT_ID MEDIUMINT UNSIGNED NOT NULL,
--    HADM_ID MEDIUMINT UNSIGNED,
--    ICUSTAY_ID MEDIUMINT UNSIGNED,
...
--    COMMENTS_STATUS VARCHAR(255),
--    COMMENTS_TITLE VARCHAR(255),
--    COMMENTS_DATE DATETIME,
--    ORIGINALCHARTTIME DATETIME,
...
LOAD DATA LOCAL INFILE 'IOEVENTS_DATA_TABLE.csv' INTO TABLE IOEVENTS
 ...
 SET
   ...
   ENDTIME = @ENDTIME,
   ITEMID = @ITEMID,
   VOLUME = IF(@VOLUME='', NULL, @VOLUME),
   VOLUMEUOM = IF(@VOLUMEUOM='', NULL, @VOLUMEUOM),
   ...
   TOTALVOLUME = IF(@TOTALVOLUME='', NULL, @TOTALVOLUME),
   TOTALVOLUMEUOM = IF(@TOTALVOLUMEUOM='', NULL, @TOTALVOLUMEUOM),
   ...
   COMMENTS_STATUS = IF(@COMMENTS_STATUS='', NULL, @COMMENTS_STATUS),
   COMMENTS_TITLE = IF(@COMMENTS_TITLE='', NULL, @COMMENTS_TITLE),
   COMMENTS_DATE = IF(@COMMENTS_DATE='', NULL, @COMMENTS_DATE),
   ORIGINALCHARTTIME = IF(@ORIGINALCHARTTIME='', NULL, @ORIGINALCHARTTIME),
...
LOAD DATA LOCAL INFILE 'NOTEEVENTS_DATA_TABLE.csv' INTO TABLE NOTEEVENTS
   ...
   LINES TERMINATED BY '\n'
   IGNORE 1 LINES
   (@ROW_ID,@RECORD_ID,@SUBJECT_ID,@HADM_ID,@CHARTDATE,@CATEGORY,@DESCRIPTION,@CGID,@ISERROR,@TEXT)
 SET
   ROW_ID = @ROW_ID,
   RECORD_ID = @RECORD_ID,
   SUBJECT_ID = @SUBJECT_ID,
...
LOAD DATA LOCAL INFILE 'PRESCRIPTIONS_DATA_TABLE.csv' INTO TABLE PRESCRIPTIONS
 ...
 SET
   ...
   ICUSTAY_ID = IF(@ICUSTAY_ID='', NULL, @ICUSTAY_ID),
   STARTTIME = IF(@STARTTIME='', NULL, @STARTTIME),
   ENDTIME = IF(@ENDTIME='', NULL, @ENDTIME),
   DRUG_TYPE = @DRUG_TYPE,
...
--    SUBJECT_ID MEDIUMINT UNSIGNED NOT NULL,
--    HADM_ID MEDIUMINT UNSIGNED NOT NULL,
--    PROC_SEQ_NUM TINYINT UNSIGNED NOT NULL,
--    ICD9_CODE SMALLINT UNSIGNED NOT NULL);
...
LOAD DATA LOCAL INFILE 'PROCEDURES_ICD_DATA_TABLE.csv' INTO TABLE PROCEDURES_ICD
   ...
   LINES TERMINATED BY '\n'
   IGNORE 1 LINES
   (@ROW_ID,@SUBJECT_ID,@HADM_ID,@PROC_SEQ_NUM,@ICD9_CODE)
 SET
   ROW_ID = @ROW_ID,
   SUBJECT_ID = @SUBJECT_ID,
   HADM_ID = @HADM_ID,
   PROC_SEQ_NUM = @PROC_SEQ_NUM,
   ICD9_CODE = @ICD9_CODE;
