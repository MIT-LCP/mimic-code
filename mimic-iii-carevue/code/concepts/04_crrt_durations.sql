...
(
  select 
    ...
    , max(
      case
        -- System Integrity
        when ce.itemid = 224146 and value in ('New Filter','Reinitiated') then 1
    ...
    , max(
      case
        -- System Integrity
        when ce.itemid = 224146 and value in ('Discontinued','Recirculating') then 1
        -- the only value like DC is "DC'D", use like to avoid apostrophe
        when ce.itemid = 665 and (value = 'Clotted' or value like 'DC%') then 1
  ...
  select
    ...
    , case
      ...
      when rrt_end = 1
        ...
        -- so here we output 0
        then 0
      when rrt_ended_prev_row = 1 then 1
      -- if there is less than 2 hours between CRRT settings, we do not treat this as a new CRRT event
      when round((cast(extract(epoch from (charttime - charttime_prev_row))/(60*60) as numeric)), 8) <= 2 then 0
      else 1
    end as newcrrt
...
(
  select 
    ...
    , case 
      when rrt_start = 1 or rrt=1 or rrt_end = 1 then sum(newcrrt) over (partition by icustay_id order by charttime)
      else null 
    end as num
