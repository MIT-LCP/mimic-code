...
select ie.subject_id, ie.hadm_id, ie.icustay_id
      ...
      , round((cast(extract(epoch from (ie.intime - pat.dob))/(60*60*24*365) as numeric)),0) as age
      , gcs.mingcs
      , vital.heartrate_max
      , vital.heartrate_min
      , vital.meanbp_max
      ...
      -- age group
      , case
        when (cast(extract(epoch from (ie.intime - pat.dob))/(60*60*24*365) as numeric)) <= 1 then 'neonate'
        when (cast(extract(epoch from (ie.intime - pat.dob))/(60*60*24*365) as numeric)) <= 15 then 'middle'
