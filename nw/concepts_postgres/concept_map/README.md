# Load NW Concept Maps into a PostgreSQL database

The scripts in this folder create the schema for NW concept maps and load the mapping data into the appropriate tables for PostgreSQL v10+.

Concept maps provide mappings from NW local codes to standard terminologies (LOINC, OMOP, RxNorm, SNOMED).

## Quickstart

Run from the `nw/concepts_postgres/concept_map/` directory:
...
First ensure that Postgres is running on your computer and that the database already exists.

### Step 1: Create schema and tables

Change to the `nw/concepts_postgres/concept_map/` directory. Create the schema and tables with:

```sh
psql -d mimiciv -f create.sql
```

This creates the `nw_concept_map` schema with the following tables:
...
**Note:** This will drop and recreate the `nw_concept_map` schema, deleting any existing data.
