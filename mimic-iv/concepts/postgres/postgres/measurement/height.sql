...
(
  SELECT 
    ...
    , ROUND( CAST(  CAST( c.valuenum * 2.54 as numeric),2) AS height
     as numeric),c.valuenum as height_orig
...
(
  SELECT 
    c.subject_id, c.stay_id, c.charttime
    -- Ensure that all heights are in centimeters
    , ROUND( CAST(  CAST( c.valuenum as numeric),2) AS height
  FROM mimic_icu.chartevents c
  WHERE c.valuenum IS NOT NULL
  -- Height cm
  AND c.itemid = 226730
)
-- merge cm/height as numeric),only take 1 value per charted row
, ht_stg0 AS
(
  SELECT
